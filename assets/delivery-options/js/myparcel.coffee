...
jquery(document).ready ->
  ...
  window.mypa.initialize = ->
    ...
    bindInputListeners() unless el.shadowRoot

    return unless el?
    shadow = el.createShadowRoot() unless el.shadowRoot?
  ...
  window.mypa.fn.updatePage = window.mypa.updatePage = fetchDeliveryOptions = (postal_code, number, street)->
  ...
  preparePickup = (pickupOptions) ->
    ...
    #$($('#mypa-pickup-row input[name=mypa-delivery-time]')[0]).prop 'checked', true
