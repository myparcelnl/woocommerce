MyParcelBE = {
    ...
    init: function() {
        ...
        if (MyParcelBE.data.config.deliveryTitle) {
            jQuery('#mypa-delivery-title').html(MyParcelBE.data.config.deliveryTitle);
        }
        if (MyParcelBE.data.config.headerDeliveryOptions) {
            jQuery('#mypa-delivery-options-title').html(MyParcelBE.data.config.headerDeliveryOptions);
            jQuery('#header-delivery-options-title').show();
        }
        if (MyParcelBE.data.config.onlyRecipientTitle) {
            jQuery('#mypa-only-recipient-title').html(MyParcelBE.data.config.onlyRecipientTitle);
        }
        if (MyParcelBE.data.config.signatureTitle) {
            jQuery('#mypa-signature-title').html(MyParcelBE.data.config.signatureTitle);
        }
        if (MyParcelBE.data.config.pickupTitle) {
            jQuery('#mypa-pickup-title').html(MyParcelBE.data.config.pickupTitle);
        }
    ...
    showPrices: function(prices) {
        jQuery.each(prices, function(selectName, price) {
            jQuery('#mypa-' + selectName + '-delivery, #mypa-' + selectName + '-price').html(MyParcelBE.getPriceHtml(price));
        });
    },
    ...
    bind: function() {
        ...
        jQuery('#method-myparcel-delivery-morning, #method-myparcel-delivery-evening').on('click', function() {
            MyParcelBE.defaultCheckCheckbox('mypa-only-recipient');
        });
        ...
        jQuery('#method-myparcel-normal').on('click', function() {
            MyParcelBE.defaultCheckCheckbox('method-myparcel-normal');
        });
    ...
    mapExternalWebshopTriggers: function() {
        ...
        MyParcelBE.DELIVERY_ONLY_RECIPIENT = 0;
        ...
        if (jQuery('#mypa-pickup-delivery').prop('checked') === false && jQuery('#method-myparcel-delivery-morning').prop('checked')) {
            ...
            MyParcelBE.addStyleToPrice('#mypa-morning-delivery, #mypa-only-recipient-price');
            ...
            if (jQuery('#mypa-signature-selector').prop('checked')) {
                ...
                MyParcelBE.addStyleToPrice('#mypa-signature-price');
            }
        ...
        if (jQuery('#mypa-pickup-delivery').prop('checked') === false && jQuery('#method-myparcel-normal').prop('checked')) {
            MyParcelBE.addStyleToPrice('#mypa-normal-delivery');
            ...
            if (jQuery('#mypa-signature-selector').prop('checked') && jQuery('#mypa-only-recipient-selector').prop('checked')) {
                ...
                MyParcelBE.DELIVERY_SIGNATURE = 1;
                MyParcelBE.DELIVERY_ONLY_RECIPIENT = 1;
                MyParcelBE.addStyleToPrice('#mypa-signature-price, #mypa-only-recipient-price');
            } else
            ...
            if (jQuery('#mypa-only-recipient-selector').prop('checked')) {
                ...
                MyParcelBE.DELIVERY_ONLY_RECIPIENT = 1;
                MyParcelBE.addStyleToPrice('#mypa-only-recipient-price');
        ...
        if (jQuery('#mypa-pickup-delivery').prop('checked') === false && jQuery('#method-myparcel-delivery-evening').prop('checked')) {
            ...
            MyParcelBE.DELIVERY_ONLY_RECIPIENT = 1;
            MyParcelBE.addStyleToPrice('#mypa-evening-delivery, #mypa-only-recipient-price');
        ...
        if (jQuery('#mypa-pickup-delivery').prop('checked') || jQuery('#mypa-pickup-selector').prop('checked')) {
            ...
            if (jQuery('#mypa-pickup-express-selector').prop('checked')) {
                ...
                MyParcelBE.addStyleToPrice('#mypa-pickup-express-price');
                return;
            } else {
                MyParcelBE.addStyleToPrice('#mypa-pickup-price');
            }
    ...
    addDeliveryToExternalInput: function(deliveryMomentOfDay) {
        ...
        if (currentDeliveryData !== null) {
            ...
            currentDeliveryData.only_recipient = MyParcelBE.DELIVERY_ONLY_RECIPIENT;
    ...
    defaultCheckCheckbox: function(selectedOption) {
        if (selectedOption === 'mypa-only-recipient') {
            ...
            jQuery('#mypa-only-recipient-price').html(MyParcelBE.getPriceHtml(this.data.config.priceOnlyRecipient));
        }
    },
    ...
    toggleDeliveryOptions: function() {
        ...
        if (isPickup && this.currentLocation.price_comment === MyParcelBE.DELIVERY_PICKUP_EXPRESS && this.data.config.allowPickupExpress) {
            jQuery('#mypa-pickup-express-price').html(MyParcelBE.getPriceHtml(this.data.config.pricePickupExpress));
    ...
    showDelivery: function() {
        ...
        if (MyParcelBE.data.address.cc === "NL") {
            ...
            if (this.data.config.allowOnlyRecipient) {
                MyParcelBE.showOnlyRecipient();
            }
        }

        if (MyParcelBE.data.address.cc === 'BE') {
            jQuery('#mypa-delivery-title').html(MyParcelBE.data.config.BEdeliveryTitle);
    ...
    showLocationDetails: function() {
        ...
        jQuery.each(
            currentLocation.opening_hours, function(weekday, value) {
                html += '<span class="mypa-pickup-location-details-day">' + MyParcelBE.data.translateENtoNL[weekday] + "</span>";
    ...
    showRetry: function() {
        MyParcelBE.showMessage(
            ...
            '<input type="text" name="mypa-error-postcode" id="mypa-error-postcode" value="' + MyParcelBE.data.address.postalCode + '">' +
            '</div><div class="mypa-full-width mypa-error">' +
            '<label for="mypa-error-number">Huisnummer</label>' +
            '<input type="text" name="mypa-error-number" id="mypa-error-number" value="' + MyParcelBE.data.address.number + '">' +
