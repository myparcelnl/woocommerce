PostNL = {
    ...
    init: function() {
        ...
        if (PostNL.data.textToTranslate.deliveryTitle) {
            jQuery('#post-delivery-title').html(PostNL.data.textToTranslate.deliveryTitle);
        }
        if (PostNL.data.textToTranslate.headerDeliveryOptions) {
            jQuery('#post-delivery-options-title').html(PostNL.data.textToTranslate.headerDeliveryOptions);
            jQuery('#header-delivery-options-title').show();
        }
        if (PostNL.data.textToTranslate.onlyRecipientTitle) {
            jQuery('#post-only-recipient-title').html(PostNL.data.textToTranslate.onlyRecipientTitle);
        }
        if (PostNL.data.textToTranslate.signatureTitle) {
            jQuery('#post-signature-title').html(PostNL.data.textToTranslate.signatureTitle);
        }
        if (PostNL.data.textToTranslate.pickupTitle) {
            jQuery('#post-pickup-title').html(PostNL.data.textToTranslate.pickupTitle);
            jQuery('.post-pickup-delivery-titel').html(PostNL.data.textToTranslate.pickUpFrom);
        }
        ...
        PostNL.showPrices(prices);
        PostNL.callDeliveryOptions();

        /* Engage defaults */
        PostNL.hideDelivery();
        jQuery('#method-postnl-normal').click();
    ...
    showPrices: function(prices) {
        jQuery.each(prices, function(selectName, price) {
            jQuery('#post-' + selectName + '-delivery, #post-' + selectName + '-price').html(PostNL.getPriceHtml(price));
        });
    },
    ...
    setCurrentDeliveryOptions: function() {
        ...
        var selectedDate = jQuery('#post-select-date').val();
        ...
        jQuery.each(selectDateKey, function(key, value) {
            if (value['price_comment'] == 'morning' && PostNL.data.config.allowMorningDelivery) {
                var morningTitle = PostNL.data.textToTranslate.deliveryMorningTitle;
                PostNL.getDeliveryTime(morningTitle, 'morning', value['start'], value['end']);
                PostNL.showMorningDelivery();
            }

            if (value['price_comment'] == 'standard') {
                var standardTitle = PostNL.data.textToTranslate.deliveryStandardTitle;
                if (PostNL.data.address.cc === 'BE') {
                    standardTitle = PostNL.data.textToTranslate.BEdeliveryStandardTitle;
                }
                PostNL.getDeliveryTime(standardTitle, 'standard', value['start'], value['end']);
            }
            if (value['price_comment'] == 'avond' && PostNL.data.config.allowEveningDelivery) {
                var eveningTitle = PostNL.data.textToTranslate.deliveryEveningTitle;
                PostNL.getDeliveryTime(eveningTitle, 'evening', value['start'], value['end']);
    ...
    getDeliveryTime: function(configDeliveryTitle, deliveryMoment, startTime, endTime) {
        ...
        jQuery('#post-' + deliveryMoment + '-title').html(configDeliveryTitle);

        if (!configDeliveryTitle) {
            jQuery('#post-' + deliveryMoment + '-title').html(startTime + ' - ' + endTime);
        }
    },

    setCurrentLocation: function() {
        var locationId = jQuery('#post-pickup-location').val();
     ...
     */
    bind: function() {
        jQuery('#post-submit').on('click', function(e) {
            e.preventDefault();
            PostNL.exportDeliveryOptionToWebshop();
        });

        /* show default delivery options and hide PostNL options */
        jQuery('#post-select-delivery').on('click', function() {
            PostNL.setCurrentDeliveryOptions();
            PostNL.showDelivery();
            PostNL.showShippingAddress();
            PostNL.hidePickUpLocations();
        });

        /* hide default delivery options and show PostNL options */
        jQuery('#post-pickup-delivery').on('click', function() {
            PostNL.hideDelivery();
            PostNL.hideShippingAddress();
            PostNL.showPickUpLocations();
        });

        jQuery('#method-postnl-delivery-morning, #method-postnl-delivery-evening').on('click', function() {
            PostNL.defaultCheckCheckbox('post-only-recipient');
        });

        /* Mobile specific triggers */
        if (isMobile) {
            jQuery('#post-show-location-details').on('click', function() {
        ...
        jQuery('#post-location-details').on('click', function() {
            PostNL.hideLocationDetails();
        });

        jQuery('#method-postnl-normal').on('click', function() {
            PostNL.defaultCheckCheckbox('method-postnl-normal');
        });

        // jQuery('#post-pickup-express').hide();  /* todo: move */

        jQuery('#post-pickup-delivery, #post-pickup-location').on('change', function(e) {
            PostNL.setCurrentLocation();
            PostNL.toggleDeliveryOptions();
            PostNL.mapExternalWebshopTriggers();
        });

        jQuery('#post-select-date').on('change', function(e) {
            PostNL.setCurrentDeliveryOptions();
            PostNL.mapExternalWebshopTriggers();
        });

        /* External webshop triggers */
        jQuery('#post-load input, #post-load select').on('click', function() {
            PostNL.mapExternalWebshopTriggers()
        });

        fields = window.postnl_is_using_split_address_fields
    ...
    mapExternalWebshopTriggers: function() {
         ...
         *
         */
        if (jQuery('#post-pickup-delivery').prop('checked') === false && jQuery('#method-postnl-delivery-morning').prop('checked')) {
            jQuery('#s_method_postnl_morning').click();
            PostNL.DELIVERY_ONLY_RECIPIENT = 1;
            PostNL.addStyleToPrice('#post-morning-delivery, #post-only-recipient-price');

            /**
             * Signature
             */
            if (jQuery('#post-signature-selector').prop('checked')) {
                jQuery('#s_method_postnl_morning_signature').click();
                PostNL.DELIVERY_SIGNATURE = 1;
                PostNL.addStyleToPrice('#post-signature-price');
            }

            PostNL.addDeliveryToExternalInput(PostNL.DELIVERY_MORNING);
            return;
        }
         ...
         *
         */
        if (jQuery('#post-pickup-delivery').prop('checked') === false && jQuery('#method-postnl-normal').prop('checked')) {
            PostNL.addStyleToPrice('#post-normal-delivery');

            /**
             * Signature and only recipient
             */
            if (jQuery('#post-signature-selector').prop('checked') && jQuery('#post-only-recipient-selector').prop('checked')) {
                jQuery('#s_method_postnl_delivery_signature_and_only_recipient_fee').click();
                PostNL.DELIVERY_SIGNATURE = 1;
                PostNL.DELIVERY_ONLY_RECIPIENT = 1;
                PostNL.addStyleToPrice('#post-signature-price, #post-only-recipient-price');
            } else
             ...
             */
            if (jQuery('#post-signature-selector').prop('checked')) {
                jQuery('#s_method_postnl_delivery_signature').click();
                PostNL.DELIVERY_SIGNATURE = 1;
                PostNL.addStyleToPrice('#post-signature-price');
             ...
             * Only recipient
             */
            if (jQuery('#post-only-recipient-selector').prop('checked')) {
                jQuery('#s_method_postnl_delivery_only_recipient').click();
                PostNL.DELIVERY_ONLY_RECIPIENT = 1;
                PostNL.addStyleToPrice('#post-only-recipient-price');

            } else {
                jQuery('#s_method_postnl_flatrate, #s_method_postnl_tablerate').click();
            }

            PostNL.addDeliveryToExternalInput(PostNL.DELIVERY_NORMAL);
            return;
        }
         ...
         *
         */
        if (jQuery('#post-pickup-delivery').prop('checked') === false && jQuery('#method-postnl-delivery-evening').prop('checked')) {
            jQuery('#s_method_postnl_evening').click();
            PostNL.DELIVERY_ONLY_RECIPIENT = 1;
            PostNL.addStyleToPrice('#post-evening-delivery, #post-only-recipient-price');

            /**
             * Signature
             */
            if (jQuery('#post-signature-selector').prop('checked')) {
                jQuery('#s_method_postnl_evening_signature').click();
                PostNL.DELIVERY_SIGNATURE = 1;
                PostNL.addStyleToPrice('#post-signature-price');
            }

            PostNL.addDeliveryToExternalInput(PostNL.DELIVERY_EVENING);
            return;
        }
         ...
         *
         */
        if (jQuery('#post-pickup-delivery').prop('checked') || jQuery('#post-pickup-selector').prop('checked')) {
            /**
             * Early morning pickup
             */
            if (jQuery('#post-pickup-express-selector').prop('checked')) {
                jQuery('#s_method_postnl_pickup_express').click();
                PostNL.addPickupToExternalInput(PostNL.DELIVERY_PICKUP_EXPRESS);
                PostNL.addStyleToPrice('#post-pickup-express-price');
                return;
            } else {
                PostNL.addStyleToPrice('#post-pickup-price');
            }

            jQuery('#s_method_postnl_pickup').click();
            PostNL.addPickupToExternalInput(PostNL.DELIVERY_PICKUP);
        }
    },
    ...
    addPickupToExternalInput: function(selectedPriceComment) {
        ...
        if (selectedPriceComment === PostNL.DELIVERY_PICKUP) {
            result.price_comment = PostNL.DELIVERY_PICKUP;
        }

        jQuery('body').trigger('update_checkout');
        jQuery('#post-input').val(JSON.stringify(result));
    },

    addDeliveryToExternalInput: function(deliveryMomentOfDay) {
        var deliveryDateId = jQuery('#post-select-date').val();
        ...
        if (currentDeliveryData !== null) {
            currentDeliveryData.signature = PostNL.DELIVERY_SIGNATURE;
            currentDeliveryData.only_recipient = PostNL.DELIVERY_ONLY_RECIPIENT;
            jQuery('#post-input').val(JSON.stringify(currentDeliveryData));
        }
        jQuery('body').trigger('update_checkout');
    },

    addStyleToPrice: function(chosenDelivery) {
        jQuery(chosenDelivery).addClass('post-bold-price');
    },

    removeStyleFromPrice: function() {
        jQuery('.post-delivery-option-table').find("span").removeClass('post-bold-price');
    },
    ...
    triggerDefaultOptionDelivery: function(deliveryDateId, deliveryMomentOfDay) {
        ...
        if (currentDeliveryData === null) {
            jQuery('#post-only-recipient-selector').prop('disabled', false).prop('checked', false);
            jQuery('#method-postnl-normal').prop('checked', true);
            PostNL.mapExternalWebshopTriggers();
        }
    ...
    defaultCheckCheckbox: function(selectedOption) {
        if (selectedOption === 'post-only-recipient') {
            jQuery('#post-only-recipient-selector').prop('checked', true).prop({disabled: true});
            jQuery('#post-only-recipient-price').html('Inclusief');
        } else {
            jQuery('#post-only-recipient-selector').prop('checked', false).removeAttr("disabled");
            jQuery('#post-only-recipient-price').html(PostNL.getPriceHtml(this.data.config.priceOnlyRecipient));
        }
    },
    ...
    toggleDeliveryOptions: function() {
        var isPickup = jQuery('#post-pickup-delivery').is(':checked');
        jQuery('#post-pickup-selector').prop('checked', true);

        if (isPickup && this.currentLocation.price_comment === PostNL.DELIVERY_PICKUP_EXPRESS && this.data.config.allowPickupExpress) {
            jQuery('#post-pickup-express-price').html(PostNL.getPriceHtml(this.data.config.pricePickupExpress));
            jQuery('#post-pickup-express').show();
        } else {
            jQuery('#post-pickup-express-selector').attr("checked", false);
            jQuery('#post-pickup-express').hide();
        }
    },
    ...
    exportDeliveryOptionToWebshop: function() {
        var deliveryOption = "";
        var selected = jQuery("#post-delivery-option-form").find("input[type='radio']:checked");
    ...
    hideMessage: function() {
        jQuery('.post-message-model').hide();
        jQuery('#post-delivery-option-form').show();
    },
    ...
    showMessage: function(message) {
        jQuery('.post-message-model').show();
        jQuery('#post-message').html(message).show();
        jQuery('#post-delivery-option-form').hide();
    ...
    hideDelivery: function() {
        jQuery('#post-delivery-date-text,.post-extra-delivery-options').hide();
        jQuery('#post-select-date').parent().parent().hide();
        jQuery('#post-delivery').parent().parent().hide();
    ...
    showDelivery: function() {
        ...
        jQuery('#post-delivery').parent().parent().show();

        if (PostNL.data.address.cc === "NL") {
            jQuery('#post-delivery-selectors-' + this.data.address.cc.toLowerCase()).show();
            jQuery('.post-extra-delivery-options').show();

            if (this.data.config.deliverydaysWindow >= 2) {
                jQuery('#post-delivery-date-select').show();
            }
        ...
        if (PostNL.data.address.cc === 'BE') {
            jQuery('#post-delivery-title').html(PostNL.data.config.BEdeliveryTitle);
            jQuery('#post-delivery-date-text').hide();
        }
    },

    hideAllDeliveryOptions: function() {
        jQuery('#post-load').hide();
    },

    showAllDeliveryOptions: function() {
        jQuery('#post-load').show();
    },
    ...
    showSpinner: function() {
        jQuery('#post-delivery-option-form').hide();
        jQuery('.post-message-model').hide();
        jQuery('#post-spinner-model').show();
    },
    ...
    hideSpinner: function() {
        jQuery('#post-spinner-model').hide();
    },

    showMorningDelivery: function() {
        jQuery('#method-postnl-delivery-morning-div').show();
    },

    hideMorningDelivery: function() {
        jQuery('#method-postnl-delivery-morning-div').hide();
    },

    showEveningDelivery: function() {
        jQuery('#method-postnl-delivery-evening-div').show();
    },

    hideEveningDelivery: function() {
        jQuery('#method-postnl-delivery-evening-div').hide();
    },

    showSignature: function() {
        jQuery('.post-extra-delivery-option-signature, #post-signature-price').show();
    },

    hideSignature: function() {
        jQuery('.post-extra-delivery-option-signature, #post-signature-price').hide();
    },

    showOnlyRecipient: function() {
        jQuery('#post-only-recipient, #post-only-recipient-price').parent().show();
    },

    hideOnlyRecipient: function() {
        jQuery('#post-only-recipient, #post-only-recipient-price').parent().hide();
    },
    ...
    showDeliveryDates: function() {
        ...
        jQuery.each(PostNL.data.deliveryOptions.data.delivery, function(key, value) {
        ...
        if (deliveryWindow === 0) {
            jQuery('#post-delivery-date-select').hide();
        }

        /* When deliverydaysWindow is 1, hide the day selector and show a div to show the date */
        if (deliveryWindow === 1) {
            jQuery('#post-select-date').hide();
            jQuery('#post-delivery-date-text').show();
        }

        /* When deliverydaysWindow > 1, show the day selector */
        if (deliveryWindow > 1) {
            jQuery('#post-select-date').show();
        }

        jQuery('#post-select-date, #post-date').html(html);
    },

    hideDeliveryDates: function() {
        jQuery('#post-delivery-date-text').parent().hide();
    },
    ...
    clearPickUpLocations: function() {
        ...
        jQuery('#post-pickup-location').html(html);
    },
    ...
    hidePickUpLocations: function() {
        if (!PostNL.data.config.allowPickupPoints) {
            jQuery('#post-pickup-location-selector').hide();
        }

        jQuery('#post-pickup-options, #post-pickup, #post-pickup-express').hide();
    },
    ...
    showPickUpLocations: function() {
        if (false === PostNL.data.config.allowPickupPoints) {
            return;
        }

        var html = "";
        jQuery.each(PostNL.data.deliveryOptions.data.pickup, function(key, value) {
        ...
        jQuery('#post-pickup-location').html(html).prop("checked", true);
        jQuery('#post-pickup-location-selector, #post-pickup-options, #post-pickup').show();
    },
    ...
    hideLocationDetails: function() {
        jQuery('#post-delivery-option-form').show();
        jQuery('#post-location-details').hide();
    },
    ...
    retryPostalCodeHouseNumber: function() {
        var retryPostalCode = jQuery('#post-error-postcode').val();
        var retryNumber = jQuery('#post-error-number').val();

        if (window.postnl_is_using_split_address_fields) {
            jQuery('#billing_house_number').val(retryNumber);
        } else {
            address = PostNL.data.address.street + ' ' + retryNumber;
            if (typeof PostNL.data.address.numberSuffix !== 'undefined') {
                address += PostNL.data.address.numberSuffix
            }
        ...
        PostNL.callDeliveryOptions();
        jQuery('#post-select-delivery').click();
    },
    ...
    showFallBackDelivery: function() {
        PostNL.hideSpinner();
        PostNL.hideDelivery();
        jQuery('#post-select-date, #method-postnl-normal-div, .post-is-pickup-element').hide();
        jQuery('#post-select-delivery-title').html('Zo snel mogelijk bezorgen');
    },
    ...
    showRetry: function() {
        PostNL.showMessage(
            '<h3>' + PostNL.data.textToTranslate.wrongHouseNumberPostcode + '</h3>' +
            '<div class="post-full-width post-error">' +
            '<label for="post-error-postcode">' + PostNL.data.textToTranslate.postcode + '</label>' +
            '<input type="text" name="post-error-postcode" id="post-error-postcode" value="' + PostNL.data.address.postalCode + '">' +
            '</div><div class="post-full-width post-error">' +
            '<label for="post-error-number">' + PostNL.data.textToTranslate.houseNumber + '</label>' +
            '<input type="text" name="post-error-number" id="post-error-number" value="' + PostNL.data.address.number + '">' +
            '<br><div id="post-error-try-again" class="button btn">' + PostNL.data.textToTranslate.again + '</div>' +
            '</div>'
        );

        /* remove trigger that closes message */
        jQuery('#post-message').off('click');

        /* bind trigger to new button */
        jQuery('#post-error-try-again').on('click', function() {
    ...
    setAddressFromInputFields: function() {
        ...
        address = PostNL.getAddressInputValues(addressType);

        if (!PostNL.getAddressInputValues('billing').postalCode) {
            return;
        }
    ...
    getAddressInputValues: function(type) {
        ...
        if (window.postnl_is_using_split_address_fields) {
            ...
            streetParts = PostNL.splitFullStreetFromInput(input.fullStreet);
    ...
    splitFullStreetFromInput: function(fullStreet) {
        ...
        if (fullStreet.length) {
            streetParts = new RegExp(PostNL.SPLIT_STREET_REGEX).exec(fullStreet);
    ...
    callDeliveryOptions: function() {
        ...
        PostNL.hideDelivery();
        PostNL.setAddressFromInputFields();

        // Hide PostNL field if there is no address entered
        if (PostNL.data.address.postalCode == '' || PostNL.data.address.number == '') {
            PostNL.hideSpinner();
            PostNL.showMessage(
                '<h3>' + PostNL.data.textToTranslate.addressNotFound + '</h3>'
            );
            return;
        }
