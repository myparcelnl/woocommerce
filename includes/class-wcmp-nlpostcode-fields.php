...
class WC_NLPostcode_Fields {
	...
	public function add_styles_scripts(){
		if ( is_checkout() || is_account_page() ) {
			if ( version_compare( WOOCOMMERCE_VERSION, '2.1', '<=' ) ) {
				...
				wp_register_script(
					'nl-checkout',
					WooCommerce_MyParcelBE()->plugin_url() . '/assets/js/nl-checkout.js',
					array( 'wc-checkout' ),
					$this->version
				);
			...
			if ( is_account_page() ) {
				...
				wp_register_script(
					'nl-account-page',
					WooCommerce_MyParcelBE()->plugin_url() . '/assets/js/nl-account-page.js',
					array( 'jquery' ),
					$this->version
				);
			...
			wp_enqueue_style( 'nl-checkout', WooCommerce_MyParcelBE()->plugin_url() . '/assets/css/myparcel-checkout.css' );
		}
	...
	public function admin_scripts_styles ( $hook ) {
		...
		if ( $post_type == 'shop_order' ) {
			wp_enqueue_style(
				'nl-checkout-admin',
				WooCommerce_MyParcelBE()->plugin_url() . '/assets/css/nl-checkout-admin.css',
				array(), // deps
				$this->version
			);
		}
	...
	public function nl_checkout_fields( $fields, $country, $form ) {
		...
		$fields[$form.'_house_number_suffix'] = array(
			...
			// 'placeholder'	=> __( 'Suffix', 'woocommerce-myparcelbe' ),
