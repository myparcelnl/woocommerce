...
class WooCommerce_PostNL_Settings {
    ...
    public function __construct() {
        ...
        add_filter(
            'plugin_action_links_' . WooCommerce_PostNL()->plugin_basename, array(
                $this,
                'add_settings_link'
            )
        );
    ...
    public function settings_page() {
        $settings_tabs = apply_filters(
            'woocommerce_postnl_settings_tabs',
            array(
    ...
    public function postnl_be_notice() {
    ...
    public function general_settings() {
        ...
        add_settings_field(
            ...
            array(
                ...
                'description' => __('Show Track & Trace trace code and link in My Account.', 'woocommerce-postnl')
            )
        );
        ...
        add_settings_field(
            ...
            __('Title before the barcode', 'woocommerce-postnl'),
            array($this->callbacks, 'text_input'),
            $option_group,
            'general',
            array(
                ...
                'size'        => 25,
                'description' => __('You can change the text before the barcode inside an note', 'woocommerce-postnl'),
            )
        );
    ...
    public function export_defaults_settings() {
        ...
        if (version_compare(WOOCOMMERCE_VERSION, '2.1', '>=')) {
            add_settings_field(
				...
				array(
                    ...
                    'description'   => __('Select one or more shipping methods for each delivery type', 'woocommerce-postnl'),
                )
            );
        } else {
            add_settings_field(
				...
				array(
                    'option_name' => $option_name,
                    'id'          => 'package_type',
                    'default'     => (string) WooCommerce_PostNL_Export::PACKAGE,
        ...
        add_settings_field(
            ...
            array(
                'option_name' => $option_name,
                'id'          => 'signature',
                'description' => __('The parcel will be offered at the delivery address. If the recipient is not at home, the parcel will be delivered to the neighbours. In both cases, a signature will be required.', 'woocommerce-postnl')
            )
        );

        add_settings_field(
            'age_check',
            __('Age check 18+', 'woocommerce-postnl'),
            array($this->callbacks, 'checkbox'),
            $option_group,
            'defaults',
            array(
                'option_name' => $option_name,
                'id'          => 'age_check',
                'description' => __('The age check is intended for parcel shipments for which the recipient must show 18+ by means of a proof of identity. With this shipping option <strong>Signature for receipt</strong> and <strong>Delivery only at recipient</strong> are included. The age 18+ is further excluded from the delivery options morning and evening delivery.', 'woocommerce-postnl')
            )
        );
        ...
        add_settings_field(
			...
			array(
                ...
                'id'          => 'hs_code',
                'size'        => '5',
                'description' => sprintf(__('HS Codes are used for PostNL world shipments, you can find the appropriate code on the %ssite of the Dutch Customs%s.', 'woocommerce-postnl'), '<a href="http://tarief.douane.nl/arctictariff-public-web/#!/home" target="_blank">', '</a>')
            )
        );
        add_settings_field(
            'package_contents',
    ...
    public function checkout_settings() {
        ...
        add_settings_field(
            ...
            __('PostNL address fields', 'woocommerce-postnl'),
            array($this->callbacks, 'checkbox' ),
            $option_group,
            ...
            array(
                ...
                'description' => __('When enabled the checkout will use the PostNL address fields. This means there will be three separate fields for street name, number and suffix. Want to use the WooCommerce default fields? Leave this option unchecked.', 'woocommerce-postnl'),
            )
        );
        ...
        add_settings_section(
            'processing_parameters', __('Shipment processing parameters', 'woocommerce-postnl'), array(
            $this->callbacks,
            'section'
        ), $option_group
        );
        ...
        add_settings_field(
            'checkout_place', __('Checkout place', 'woocommerce-postnl'), array(
            $this->callbacks,
            'select'
                ...
                'options'     => array(
                    'woocommerce_after_checkout_billing_form'  => __('Show checkout options after billing details', 'woocommerce-postnl'),
                    'woocommerce_after_checkout_shipping_form' => __('Show checkout options after shipping details', 'woocommerce-postnl'),
                    'woocommerce_after_order_notes'            => __('Show checkout options after notes', 'woocommerce-postnl'),
                ),
                'description' => __('You can change the place of the checkout options on the checkout page. By default it will be placed after shipping details.', 'woocommerce-postnl'),
            )
        );
        ...
        add_settings_field(
            'dropoff_days', __('Drop-off days', 'woocommerce-postnl'), array(
            $this->callbacks,
            'enhanced_select'
        ), $option_group, 'processing_parameters', array(
                'option_name' => $option_name,
        ...
        add_settings_field(
            'cutoff_time', __('Cut-off time', 'woocommerce-postnl'), array(
            $this->callbacks,
            'text_input'
        ), $option_group, 'processing_parameters', array(
                'option_name' => $option_name,
        ...
        add_settings_field(
            'dropoff_delay', __('Drop-off delay', 'woocommerce-postnl'), array(
            $this->callbacks,
            'text_input'
        ), $option_group, 'processing_parameters', array(
                'option_name' => $option_name,
                ...
                'type'        => 'text',
                'size'        => '5',
                'description' => __('Number of days you need to process an order.', 'woocommerce-postnl'),
            )
        );

        add_settings_field(
            'deliverydays_window', __('Delivery days window', 'woocommerce-postnl'), array(
            $this->callbacks,
            'text_input'
        ), $option_group, 'processing_parameters', array(
                'option_name' => $option_name,
                ...
                'type'        => 'text',
                'size'        => '5',
                'description' => __('Amount of days a customer can postpone delivery. Default is 0 days with a maximum of 14.', 'woocommerce-postnl'),
            )
        );

        // Delivery options section.
        add_settings_section(
            'delivery_options', __('Delivery options', 'woocommerce-postnl'), array(
            $this->callbacks,
            'section'
        ), $option_group
        );
        ...
        add_settings_field(
            'header_delivery_options_title', __('Delivery options title', 'woocommerce-postnl'), array(
            $this->callbacks,
            'text_input'
        ...
        add_settings_field(
            'at_home_delivery', __('Home delivery title', 'woocommerce-postnl'), array(
            $this->callbacks,
            'text_input'
        ), $option_group, 'delivery_options', array(
                'option_name' => $option_name,
        ...
        add_settings_field(
            'standard', __('Standard delivery title', 'woocommerce-postnl'), array(
            $this->callbacks,
            'text_input'
        ), $option_group, 'delivery_options', array(
                'option_name' => $option_name,
                ...
                'description' => __('When there is no title, the delivery time will automatically be visible.', 'woocommerce-postnl'),
            )
        );

        add_settings_field(
            'morning', __('Morning delivery', 'woocommerce-postnl'), array(
            $this->callbacks,
            'delivery_option_enable'
                ...
                'size'               => 30,
                'option_description' => __('When there is no title, the delivery time will automatically be visible.', 'woocommerce-postnl'),
            )
        );

        add_settings_field(
            'evening', __('Evening delivery', 'woocommerce-postnl'), array(
            $this->callbacks,
            'delivery_option_enable'
        ), $option_group, 'delivery_options', array(
                'has_title'          => true,
        ...
        add_settings_field(
            'only_recipient', __('Home address only', 'woocommerce-postnl'), array(
            $this->callbacks,
            'delivery_option_enable'
        ), $option_group, 'delivery_options', array(
                'has_title'   => true,
        ...
        add_settings_field(
            'signature', __('Signature on delivery', 'woocommerce-postnl'), array(
            $this->callbacks,
            'delivery_option_enable'
        ), $option_group, 'delivery_options', array(
                'has_title'   => true,
        ...
        add_settings_field(
            'pickup', __('PostNL pickup', 'woocommerce-postnl'), array(
            $this->callbacks,
            'delivery_option_enable'
        ), $option_group, 'delivery_options', array(
                'has_title'   => true,
        ...
        add_settings_field(
            'pickup_express', __('Early PostNL pickup', 'woocommerce-postnl'), array(
            $this->callbacks,
            'delivery_option_enable'
        ), $option_group, 'delivery_options', array(
                'has_title'   => false,
                ...
                'current'     => __('Early PostNL pickup', 'woocommerce-postnl'),
                'size'        => 30,
            )
        );

        add_settings_field(
            'saturday_cutoff', __('Enable monday delivery', 'woocommerce-postnl'), array(
            $this->callbacks,
            'delivery_option_enable'
        ), $option_group, 'delivery_options', array(
                'has_title'          => false,
        ...
        add_settings_section(
            'Belgium_delivery_options', __('Belgium delivery options', 'woocommerce-postnl'), array(
            $this->callbacks,
            'section'
        ), $option_group
        );
        ...
        add_settings_field(
            'belgium_at_home_delivery', __('Belgium home delivery title', 'woocommerce-postnl'), array(
            $this->callbacks,
            'text_input'
        ...
        add_settings_field(
            'BEstandard', __('Belgium standard delivery title', 'woocommerce-postnl'), array(
            $this->callbacks,
            'text_input'
        ), $option_group, 'Belgium_delivery_options', array(
                'option_name' => $option_name,
        ...
        add_settings_section(
            'customizations', __('Customizations', 'woocommerce-postnl'), array(
            $this->callbacks,
            'section'
        ), $option_group
        );
        ...
        add_settings_field(
            'custom_css', __('Custom styles', 'woocommerce-postnl'), array(
            $this->callbacks,
            'textarea'
    ...
    public static function get_checkout_setting_title($key) {
        ...
        return __($setting, 'woocommerce-postnl');
    }
