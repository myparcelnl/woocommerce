...
class WooCommerce_PostNL_Admin {
    ...
    public function admin_order_actions($order) {
        ...
        $target = (isset(WooCommerce_PostNL()->general_settings['download_display'])
                   && WooCommerce_PostNL()->general_settings['download_display'] == 'display') ? 'target="_blank"'
            : '';
        $nonce = wp_create_nonce('wc_postnl');
        foreach ($listing_actions as $action => $data) {
            printf(
                '<a href="%1$s" class="button tips postnl %2$s" alt="%3$s" data-tip="%3$s" data-order-id="%4$s" data-request="%2$s" data-nonce="%5$s" %6$s>',
                $data['url'],
                $action,
                $data['alt'],
                $order_id,
        ...
        ?>
        <img src="<?php echo WooCommerce_PostNL()->plugin_url(
    ...
    public function create_box_content() {
        ...
        if ( ! empty($consignments)) {
            ...
            <table class="tracktrace_status">
                ...
                $action = 'get_labels';
                $target = (isset(WooCommerce_PostNL()->general_settings['download_display'])
                           && WooCommerce_PostNL()->general_settings['download_display'] == 'display')
                    ? 'target="_blank"'
                    : '';
                $nonce = wp_create_nonce('wc_postnl');
                    ...
                    <tr>
                        <td class="wcmp-create-label">
                            ...
                            printf(
                                ...
                                WooCommerce_PostNL()->plugin_url() . "/assets/img/postnl-pdf.png",
                                $label_button_text
                            );
                            printf("</a>");
                            ?>
    ...
    public function show_order_delivery_options($order) {
        ...
        if ( ! empty($date)
            ...
            printf(
                ...
                __('Delivery date', 'woocommerce-postnl'),
                $formatted_date,
                $time_title
            );
        }
    ...
    public function product_hs_code_field() {
        ...
        woocommerce_wp_text_input(
            array(
                ...
                'description' => sprintf(
                    __('HS Codes are used for PostNL world shipments, you can find the appropriate code on the %ssite of the Dutch Customs%s.', 'woocommerce-postnl'),
    ...
    public function product_hs_code_field_save($post_id) {
        ...
        if (isset($_POST['_postnl_hs_code']) && ! is_array($_POST['_postnl_hs_code'])) {
            ...
            if ( ! empty($hs_code)) {
                ...
                if (isset($_POST['_postnl_hs_code']) && empty($hs_code)) {
