...
class WooCommerce_PostNL_Admin {
	...
	public function order_list_shipment_options( $order, $hide = true ) {
		...
		if ( !empty( $consignments ) )  {
			...
			$shipment = WooCommerce_PostNL()->export->get_shipment_data( $last_shipment_id, $order );
	...
	public function bulk_actions() {
		...
		if ( 'shop_order' == $post_type ) {
			...
			<img src="<?php echo WooCommerce_PostNL()->plugin_url() . '/assets/img/wpspin_light.gif';?>" class="wcmp_bulk_spinner waiting" style="display:none;"/>
		<?php
		}
	}
	...
	public function offset_dialog () {
		...
		if ( 'shop_order' == $post_type ) {
			?>
			<div id="wcpostnl_offset_dialog" style="display:none;">
				<?php _e( 'Labels to skip', 'woocommerce-postnl' ); ?>:
				<input type="text" size="2" class="wc_postnl_offset">
				<img src="<?php echo WooCommerce_PostNL()->plugin_url() . '/assets/img/print-offset-icon.png'; ?>" id="wcpostnl-offset-icon" style="vertical-align: middle;">
				<button class="button" style="display:none; margin-top: 4px"><?php _e( 'Print', 'woocommerce-postnl' ); ?></button>
			</div>
			<?php
		}
	}
	...
	public function admin_order_actions( $order ) {
		...
		foreach ($listing_actions as $action => $data) {
			printf( '<a href="%1$s" class="button tips postnl %2$s" alt="%3$s" data-tip="%3$s" data-order-id="%4$s" data-request="%2$s" data-nonce="%5$s" %6$s>', $data['url'], $action, $data['alt'], $order_id, $nonce, $target );
			?>
		...
		?>
		<img src="<?php echo WooCommerce_PostNL()->plugin_url() . '/assets/img/wpspin_light.gif';?>" style="width: 17px; margin: 5px 3px;" class="wcmp_spinner waiting"/>
		<?php
	}
	...
	public function save_shipment_options_ajax () {
		...
		if (isset($form_data['postnl_options'][$order_id])) {
			...
			if (isset($shipment_options['extra_options'])) {
				WCX_Order::update_meta_data( $order, '_postnl_shipment_options_extra', $shipment_options['extra_options'] );
	...
	public function create_box_content() {
		...
		if ( !empty( $consignments ) )  {
			...
			<table class="tracktrace_status">
				...
				$target = ( isset(WooCommerce_PostNL()->general_settings['download_display']) && WooCommerce_PostNL()->general_settings['download_display'] == 'display') ? 'target="_blank"' : '';
				$nonce = wp_create_nonce('wc_postnl');
				$label_button_text = esc_attr__( 'Print PostNL label', 'woocommerce-postnl' );
				foreach ($consignments as $shipment_id => $shipment):
					...
					if (isset($shipment['tracktrace'])) {
						...
						$tracktrace_link = '('.WooCommerce_PostNL()->export->get_package_name($shipment['shipment']['options']['package_type']).')';
					} else {
					...
					<tr>
						<td class="wcmp-create-label">
							<?php printf( '<a href="%1$s" class="button tips postnl %2$s" alt="%3$s" data-tip="%3$s" data-order-id="%4$s" data-request="%2$s" data-nonce="%5$s" %6$s>', $label_url, $action, $label_button_text, $order_id, $nonce, $target ); ?>
								<img src="<?php echo WooCommerce_PostNL()->plugin_url(); ?>/assets/img/postnl-pdf.png" alt="<?php $label_button_text; ?>" width="16" class="wcmp_button_img">
							</a>
						</td>
	...
	public function single_order_shipment_options( $order ) {
		...
		echo '<div style="clear:both;"><strong>' . __( 'PostNL shipment:', 'woocommerce-postnl' ) . '</strong><br/>';
	...
	public function show_order_delivery_options($order) {
		...
		if (!empty($date) && !(isset(WooCommerce_PostNL()->checkout_settings['deliverydays_window']) && WooCommerce_PostNL()->checkout_settings['deliverydays_window'] == 0)) {
			$formatted_date = date_i18n( apply_filters( 'wcpostnl_delivery_date_format', wc_date_format() ), strtotime( $date ) );
			if (!empty($time)) {
			...
			printf('<div class="delivery-date"><strong>%s: </strong>%s %s</div>', __('Delivery date', 'woocommerce-postnl'), $formatted_date, $time_title );
		}
		...
		if ( $pickup = WooCommerce_PostNL()->export->is_pickup( $order, $delivery_options ) ) {
			switch ($pickup['price_comment']) {
				case 'retail':
					...
					break;
				case 'retailexpress':
					$title = __( 'PostNL Pickup Express', 'woocommerce-postnl' );
					break;
			}
	...
	public function product_hs_code_field() {
		...
		woocommerce_wp_text_input( 
			array( 
				...
				'description' => sprintf( __( 'HS Codes are used for PostNL world shipments, you can find the appropriate code on the %ssite of the Dutch Customs%s.', 'woocommerce-postnl' ), '<a href="http://tarief.douane.nl/tariff/index.jsf" target="_blank">', '</a>' ),
				// 'desc_tip'    => true,
			)
		);  
		echo '</div>';
	...
	public function product_hs_code_field_save( $post_id ) {
		...
		if (isset($_POST['_postnl_hs_code']) && !is_array($_POST['_postnl_hs_code'])) {
			$product = wc_get_product( $post_id );
			$hs_code = $_POST['_postnl_hs_code'];
			if( !empty( $hs_code ) ) {
				WCX_Product::update_meta_data( $product, '_postnl_hs_code', esc_attr( $hs_code ) );
			} elseif( isset($_POST['_postnl_hs_code']) && empty( $hs_code ) ) {
				WCX_Product::delete_meta_data( $product, '_postnl_hs_code' );
			}
		}
...
return new WooCommerce_PostNL_Admin();