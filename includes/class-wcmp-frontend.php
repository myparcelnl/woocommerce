...
class WooCommerce_PostNL_Frontend {
    ...
    function __construct() {
        ...
        if (isset(WooCommerce_PostNL()->checkout_settings['postnl_checkout'])) {
            ...
            if (isset(WooCommerce_PostNL()->checkout_settings['checkout_position'])) {
                $checkout_place = WooCommerce_PostNL()->checkout_settings['checkout_position'];
            } else {
            ...
            add_action(apply_filters('wc_postnl_delivery_options_location', $checkout_place), array($this, 'output_delivery_options'), 10);
        }
    ...
    public function inject_delivery_options_variables() {
        wp_localize_script(
            ...
            array(
                ...
                'always_display' => $this->postnl_delivery_options_always_display()
            )
        );
    ...
    public function get_shipping_data() {
        if ($shipping_class = $this->get_cart_shipping_class()) {
            $shipping_data = sprintf(
                '<input type="hidden" value="%s" id="postnl_highest_shipping_class" name="postnl_highest_shipping_class">',
                $shipping_class
            );
    ...
    public function save_delivery_options($order_id, $posted) {
        ...
        if ($_POST['postnl_highest_shipping_class'] != null) {
            WCX_Order::update_meta_data(
                $order,
                '_postnl_highest_shipping_class',
                $_POST['postnl_highest_shipping_class']
            );
        } else {
        ...
        if (isset($_POST['postnl-signature-selector'])) {
            WCX_Order::update_meta_data($order, '_postnl_signature', self::RADIO_CHECKED);
        }

        if (isset($_POST['method-postnl-only-recipient-selector'])) {
    ...
    public function get_cart_shipping_class() {
        ...
        $highest_class = WooCommerce_PostNL()->export->get_shipping_class(
            $shipping_method,
    ...
    private function get_checkout_config() {
        $PostNLConfig = [
            ...
            "config" => [
                "apiBaseUrl" => WooCommerce_PostNL_Frontend_Settings::BASE_URL,
                "carrier" => "1",
                ...
                "allowMorningDelivery" => isset(WooCommerce_PostNL()->export_defaults['age_check']) ? 0 : $this->frontend_settings->is_enabled('morning'),
                "allowEveningDelivery" => isset(WooCommerce_PostNL()->export_defaults['age_check']) ? 0 : $this->frontend_settings->is_enabled('evening'),
                "allowSignature" => isset(WooCommerce_PostNL()->export_defaults['age_check']) ? 0 : $this->frontend_settings->is_enabled('signature'),
                "allowOnlyRecipient" => isset(WooCommerce_PostNL()->export_defaults['age_check']) ? 0 : $this->frontend_settings->is_enabled('only_recipient'),
            ...
            "textToTranslate" =>[
                "deliveryTitle"=> __($this->frontend_settings->get_title('at_home_delivery'), 'woocommerce-postnl'),
                "deliveryMorningTitle" => __($this->frontend_settings->get_title('morning'),'woocommerce-postnl'),
                "deliveryStandardTitle" => __($this->frontend_settings->get_title('standard'),'woocommerce-postnl'),
                "deliveryEveningTitle" => __($this->frontend_settings->get_title('evening'),'woocommerce-postnl'),
                "signatureTitle"=> __($this->frontend_settings->get_title('signature'), 'woocommerce-postnl'),
                "onlyRecipientTitle"=> __($this->frontend_settings->get_title('only_recipient'), 'woocommerce-postnl'),
                "saturdayDeliveryTitle"=> __('saturday_delivery_title', 'woocommerce-postnl'),
                "pickupTitle"=> __($this->frontend_settings->get_title('pickup'), 'woocommerce-postnl'),
                "headerDeliveryOptions" => __($this->frontend_settings->get_title('header_delivery_options'), 'woocommerce-postnl'),

                "BEdeliveryTitle" => __($this->frontend_settings->get_title('belgium_at_home_delivery'), 'woocommerce-postnl'),
                "BEdeliveryStandardTitle" => __($this->frontend_settings->get_title('belgium_standard'), 'woocommerce-postnl'),
                ...
                'monday'=> __('Monday', 'woocommerce-postnl'),
                'tuesday'=> __('Tuesday', 'woocommerce-postnl'),
                'wednesday'=> __('Wednesday', 'woocommerce-postnl'),
                'thursday'=> __('Thursday', 'woocommerce-postnl'),
                'friday'=> __('Friday', 'woocommerce-postnl'),
                'saturday'=> __('Saturday', 'woocommerce-postnl')
            ]
        ];
    ...
    private function add_fee_from_setting($post_data, $post_data_value, $delivery_type, $backend_setting, $delivery_title) {
        ...
        if (isset($post_data[$post_data_value]) && $post_data[$post_data_value] == $delivery_type) {
            if ( ! empty(WooCommerce_PostNL()->checkout_settings[$backend_setting])) {
                $fee = WooCommerce_PostNL()->checkout_settings[$backend_setting];
                $fee_name = __($delivery_title, 'woocommerce-postnl');
                $this->add_fee($fee_name, $fee);
    ...
    private function add_fee_signature($delivery_options, $delivery_title) {
        ...
        $fee = WooCommerce_PostNL()->checkout_settings['signature_fee'];
    ...
    private function get_delivery_options_shipping_methods() {
        if (isset(
                WooCommerce_PostNL()->export_defaults['shipping_methods_package_types']
            )
            && isset(WooCommerce_PostNL()->export_defaults['shipping_methods_package_types'][WooCommerce_PostNL_Export::PACKAGE])) {
            // Shipping methods associated with parcels = enable delivery options
            $delivery_options_shipping_methods = WooCommerce_PostNL()->export_defaults['shipping_methods_package_types'][WooCommerce_PostNL_Export::PACKAGE];
        } else {
    ...
    private function postnl_delivery_options_always_display() {
        if (isset(WooCommerce_PostNL()->checkout_settings['checkout_display'])
            && WooCommerce_PostNL()->checkout_settings['checkout_display'] == 'all_methods') {
            return true;
        }
