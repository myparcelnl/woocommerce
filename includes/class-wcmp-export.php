...
class WooCommerce_MyParcelBE_Export {
        ...
        if (isset($_GET['myparcelbe'])) {
            switch($_GET['myparcelbe']) {
                case 'no_consignments':
                    $message = __('You have to export the orders to MyParcel BE before you can print the labels!', 'woocommerce-myparcelbe');
    ...
    public function export() {
        ...
        if (apply_filters('wc_myparcelbe_check_privs', ! current_user_can('manage_woocommercebe_orders') && ! current_user_can('edit_shop_orders'))) {
    ...
    public function add_shipments($order_ids, $process = false) {
        ...
        if ( ! empty($this->success)) {
            $return['success'] = sprintf(__('%s shipments successfully exported to MyParcel BE', 'woocommerce-myparcelbe'), count($this->success));
        ...
        if ( $shipping_country == 'BE' ) {
            ...
            if ($pgaddress = WCX_Order::get_meta($order, '_myparcelbe_pgaddress')) {
                ...
                if (!$is_using_old_fields) {
                    ...
                    $address_intl['box_number'] = array_key_exists('_shipping_box_number', $address_parts) // optional
                        ? (string) $address_parts['box_number']
                        : '';
                    ...
                    $address_intl['box_number'] = (string) WCX_Order::get_meta($order, '_shipping_box_number');
                }
            } else {
                $address_intl = array(
                ...
                if (!$is_using_old_fields) {
                    ...
                    $address_intl['street'] = (string) $address_parts['street'];
                    $address_intl['number'] = (string) $address_parts['number'];
                    $address_intl['box_number'] = array_key_exists('box_number', $address_parts) // optional
                        ? (string) $address_parts['box_number']
                        : '';
                } else {
