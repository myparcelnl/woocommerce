...
class WCMYPA_Admin
{
    ...
    public function __construct()
    {
        ...
        add_action('woocommerce_order_note_added', [$this, 'addOrderNote'], 2, 2);
    ...
    public static function renderPickupLocation(DeliveryOptions $deliveryOptions): void
    {
        ...
        printf(
            ...
            esc_html__('Pickup location', 'woocommerce-myparcel'),
    ...
    public static function renderTrackTraceLink(array $shipment, int $order_id): void
    {
        ...
        } elseif (isset($shipment['shipment']['options'])) {
     ...
     * @return void
     */
    public function addOrderNote(int $commentId, WC_Order $order): void
    {
    ...
    public function createMetaBox(): void
    {
        ...
        $downloadDisplay =
            WCMYPA()->setting_collection->getByName(WCMYPA_Settings::SETTING_DOWNLOAD_DISPLAY) === 'display';
        $consignments    = self::get_order_shipments($order);
    ...
    {
        if (is_admin()
            && 'shop_order' === filter_input(
                INPUT_GET,
                'post_type'
            )
            && $this->anyActiveCarrierHasShowDeliveryDate()) {
            $selected = (filter_input(INPUT_GET, 'deliveryDate'))
                ? sanitize_text_field(filter_input(INPUT_GET, 'deliveryDate'))
                : false;
            ?>
          ...
          <select name="deliveryDate">
            <option value=""><?php
              ...
              $deliveryDayWindow =
                  (int) WCMYPA()->setting_collection->where('carrier', $carrierName)
                      ->getByName(
    ...
    public function generateEmailConfirmation(?array $options): ?string
    {
        if ($options) {
            $htmlHeader =
                "<h2 class='woocommerce-column__title'> " . __(
        ...
        if ($hasDeliveryDate && in_array($typenow, wc_get_order_types('order-meta-boxes'))) {
            $deliveryDate['meta_query'] = [
                [
                    ...
                    'value'   => sanitize_text_field(filter_input(INPUT_GET, 'deliveryDate')),
                    'compare' => '=',
                ],
            ];
        }
    ...
    public function getMyParcelBulkActions(): array
    {
        ...
        if (WCMP_Settings_Data::EXPORT_MODE_SHIPMENTS === $exportMode) {
            ...
            $returnValue[self::BULK_ACTION_EXPORT_PRINT] =
                __('myparcel_bulk_action_export_print', 'woocommerce-myparcel');
        }
    ...
    {
        $variation[self::META_COUNTRY_OF_ORIGIN_VARIATION] =
            get_post_meta($variation['variation_id'], self::META_COUNTRY_OF_ORIGIN_VARIATION, true);
    ...
    {
        $variation[self::META_HS_CODE_VARIATION] =
            get_post_meta($variation['variation_id'], self::META_HS_CODE_VARIATION, true);
    ...
    public function renderBarcodes(WC_Order $order): void
    {
        ...
        if (WCMP_Settings_Data::EXPORT_MODE_PPS === $exportMode) {
            ...
            esc_html_e('export_hint_no_label_created_yet', 'woocommerce-myparcel');
        ...
        foreach ($shipments as $shipment_id => $shipment) {
            ...
            $printedStatuses  =
                [WCMYPA_Admin::ORDER_STATUS_PRINTED_DIGITAL_STAMP, WCMYPA_Admin::ORDER_STATUS_PRINTED_LETTER];

            if (in_array($shipmentStatusId, $printedStatuses)) {
                echo esc_html(__('The label has been printed.', 'woocommerce-myparcel')), '<br/>';
                continue;
            }

            if (empty($shipment['track_trace'])) {
                echo esc_html(__('Concept created but not printed.', 'woocommerce-myparcel')), '<br/>';
                continue;
            }
          ...
          <div>
            ...
            <div>
              <a
                ...
                target="_blank">
                  <?php
                  esc_html_e('Print', 'woocommerce-myparcel'); ?><?php
                  self::renderSpinner(); ?>
              </a>
            </div>
          </div>
    ...
    public function showMyParcelSettings(WC_Order $order): void
    {
        try {
            ...
            WCMP_Log::add(
                sprintf('Could not get OrderSettings for order %d', $order->get_id()),
        ...
        printf(
            ...
            $order->get_id(),
            $packageTypeHuman,
            AbstractConsignment::PACKAGE_TYPE_DIGITAL_STAMP_NAME === $packageType ? sprintf(
                ' (%s - %s)',
                esc_html($digitalStampRange['min']),
                esc_html($digitalStampRange['max'])
            ) : ''
        );
    ...
    private function getConfirmationData(DeliveryOptions $deliveryOptions): ?array
    {
        ...
        if (WCMYPA()->setting_collection->isEnabled(WCMYPA_Settings::SETTING_SHOW_DELIVERY_DAY)) {
            $confirmationData[__('Date:', 'woocommerce')] =
                wc_format_datetime(new WC_DateTime($deliveryOptions->getDate() ?? ''));
        }
    ...
    private function getDeliveryTypeOptions(DeliveryOptions $deliveryOptions): string
    {
        ...
        switch ($deliveryType) {
            ...
            case AbstractConsignment::DELIVERY_TYPE_MORNING_NAME:
                $deliveryTitle =
                    WCMP_Checkout::getDeliveryOptionsTitle(WCMYPA_Settings::SETTING_MORNING_DELIVERY_TITLE);
                break;
            case AbstractConsignment::DELIVERY_TYPE_EVENING_NAME:
                $deliveryTitle =
                    WCMP_Checkout::getDeliveryOptionsTitle(WCMYPA_Settings::SETTING_EVENING_DELIVERY_TITLE);
                break;
