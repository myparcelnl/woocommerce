 ...
 *  }
 * } wcmpbe.actions
 * @property {String} wcmpbe.api_url - The API Url we use in MyParcel requests.
 * @property {String} wcmpbe.ajax_url
 * @property {String} wcmpbe.ask_for_print_position
 * @property {Object} wcmpbe.bulk_actions
 * @property {String} wcmpbe.download_display
 * @property {String} wcmpbe.nonce
 * @property {Object.<String, String>} wcmpbe.strings
 */
...
jQuery(($) => {
   ...
   * @type {Boolean}
   */
  const askForPrintPosition = Boolean(parseInt(wcmpbe.ask_for_print_position));

  const skeletonHtml
    = `<table class="wcmpbe__skeleton-loader">${
      '<tr><td><div></div></td><td><div></div></td></tr>'.repeat(5)
    }</table>`;

  const selectors = {
    bulkSpinner: '.wcmpbe__bulk-spinner',
    notice: '.wcmpbe__notice',
    offsetDialog: '.wcmpbe__offset-dialog',
    offsetDialogButton: '.wcmpbe__offset-dialog__button',
    offsetDialogClose: '.wcmpbe__offset-dialog__close',
    offsetDialogInputOffset: '.wcmpbe__offset-dialog__offset',
    orderAction: '.wcmpbe__action',
    orderActionImage: '.wcmpbe__action__img',
    printQueue: '.wcmpbe__print-queue',
    printQueueOffset: '.wcmpbe__print-queue__offset',
    shipmentOptions: '.wcmpbe__shipment-options',
    shipmentOptionsDialog: '.wcmpbe__shipment-options-dialog',
    shipmentOptionsSaveButton: '.wcmpbe__shipment-options__save',
    shipmentOptionsShowButton: '.wcmpbe__shipment-options__show',
    shipmentSettingsWrapper: '.wcmpbe__shipment-settings-wrapper',
    shipmentSummaryList: '.wcmpbe__shipment-summary__list',
    showShipmentSummaryList: '.wcmpbe__shipment-summary__show',
    spinner: '.wcmpbe__spinner',
    toggle: '.wcmpbe__toggle',
  ...
  function checkDependenciesRecursively(dependencies, input, level, queue) {
    ...
    dependencies[input.name]
      .forEach((dependency) => {
        ...
        queue.push({
          name: dependency.node.name.replace(/myparcelbe_options\[\d+\]/, ''),
          parent: input,
  ...
  function showShipmentOptionsForm(event) {
    ...
    doRequest.bind(this)({
      url: wcmpbe.ajax_url,
      data: {
        action: 'wcmpbe_get_shipment_options',
        orderId: orderId,
        security: wcmpbe.nonce,
      },
      onStart() {
  ...
  function saveShipmentOptions() {
    ...
    doRequest.bind(this)({
      url: wcmpbe.ajax_url,
      data: {
        action: 'wcmpbe_save_shipment_options',
  ...
  function doBulkAction(event) {
     ...
     */
    if (!Object.values(wcmpbe.bulk_actions).includes(action)) {
      return;
    }
    ...
    $(rows.join(', ')).addClass('wcmpbe__loading');

    if (!orderIds.length) {
      showAdminNotice(wcmpbe.strings.no_orders_selected);
      return;
    }

    switch (action) {
      // Export orders.
      case wcmpbe.bulk_actions.export:
        exportToMyParcel.bind(spinnerWrapper)(orderIds);
        break;

      // Print labels.
      case wcmpbe.bulk_actions.print:
      ...
      // Export and print.
      case wcmpbe.bulk_actions.export_print:
  ...
  function onActionClick(event) {
    ...
    if (!wcmpbe.actions.hasOwnProperty(request)) {
      return;
    }
    ...
    switch (request) {
      case wcmpbe.actions.add_shipments:
        exportToMyParcel.bind(this)();
        break;
      case wcmpbe.actions.get_labels:
        if (askForPrintPosition && !$(this).hasClass('wcmpbe__offset-dialog__button')) {
          showOffsetDialog.bind(this)();
        } else {
          printLabel.bind(this)();
        }
        break;
      case wcmpbe.actions.add_return:
  ...
  function showBulkOffsetDialog() {
    if ([wcmpbe.bulk_actions.print, wcmpbe.bulk_actions.export_print].indexOf(this.value) === -1) {
      hideOffsetDialog();
      return;
    }
  ...
  function exportToMyParcel(orderIds, print) {
    ...
    if (this.href) {
      ...
      data = {
        action: wcmpbe.actions.export,
        request: wcmpbe.actions.add_shipments,
        offset: getPrintOffset(),
        order_ids: orderIds,
        print: print,
        _wpnonce: wcmpbe.nonce,
      };
    }
    ...
    doRequest.bind(this)({
      ...
      data: data || {},
      afterDone(response) {
        const redirectUrl = updateUrlParameter(window.location.href, 'myparcelbe_done', 'true');
  ...
  function showDialog(orderIds, dialog) {
    const data = {
      action: wcmpbe.actions.export,
      request: wcmpbe.actions.modal_dialog,
      order_ids: orderIds,
      dialog,
      _wpnonce: wcmpbe.nonce,
    ...
    const url = `${wcmpbe.ajax_url}?${$.param(data)}`;

    /* disable background scrolling */
    $('body').css({overflow: 'hidden'});

    tb_show(wcmpbe.strings.dialog[dialog], url);
  }
  ...
  function printLabel(data) {
    ...
    if (this && this.href) {
      ...
      request = {
        data: {
          action: wcmpbe.actions.export,
          request: wcmpbe.actions.get_labels,
          offset: getPrintOffset(),
          _wpnonce: wcmpbe.nonce,
          ...data,
        },
    ...
    addCallback(request, 'afterDone', (response) => {
      const isDisplay = wcmpbe.download_display === 'display';
      const isDownload = wcmpbe.download_display === 'download';
  ...
  function handlePDF(request) {
    ...
    if (request.hasOwnProperty('data')) {
      url = `${wcmpbe.ajax_url}?${$.param(request.data)}`;
    } else {
      url = request.url;
    }
  ...
  function showShipmentSummaryList() {
    ...
    if (summaryList.data('loaded') === '') {
      ...
      const data = {
        security: wcmpbe.nonce,
        action: 'wcmpbe_get_shipment_summary_status',
