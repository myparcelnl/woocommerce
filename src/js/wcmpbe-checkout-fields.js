...
window.addEventListener('load', () => {
  ...
  if (!window.hasOwnProperty('MyParcel_Frontend')) {
    setTimeout(() => {
      ...
      document
        .querySelectorAll('.country_to_state')
        .forEach((selector) => {
  ...
  function setAddress(event) {
    ...
    document.querySelector(`#${type}_street_name`).value = street || '';
    document.querySelector(`#${type}_house_number`).value = number || '';
    document.querySelector(`#${type}_house_number_suffix`).value = suffix || '';
